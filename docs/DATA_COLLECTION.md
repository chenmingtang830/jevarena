...
The legacy private-research form is opt-in and separately gated from model calls. The new **public contribution mode**, when enabled, starts with a visible checkbox on before a guest answer or paid-match confirmation. Completing an answer or vote while it is on submits the displayed task and observations for research and publication after review. Turning it off before proceeding prevents that contribution upload. Page load, provider connection, JSON export and share-link creation do not submit research data. Public mode is separately deployment-gated and must not be represented as live until its migration and runtime checks pass. This intake is **not an instant public feed or a trusted benchmark**.
...
The example above is the unchanged private contract. For the gated public-candidate contract, replace `consent` with `{version: "2026-09-19-public-v1", research: true, rights: true, reviewed: true, allowPublication: true, publication: "after-review"}`. Public candidates still have no public-read endpoint and no automatic publication process. A receipt withdraws the intake payload, not a GitHub commit or third-party copy. Reviewed published cases may persist beyond the pending-data retention period; removal requests for our published copy go to the private operator contact, never to a public issue containing the deletion token.
...
Before enabling production, verify live anonymous table/RPC denial, service-role submission, duplicate behavior, wrong-token denial, withdrawal, unchanged payload count after retry, the five-minute purge schedule, and fresh below-threshold storage state. For public mode, verify the new consent is rejected while its gate is off, accepted only with the exact after-review scope when on, and that opt-out causes no upload. Use synthetic public test content, then withdraw it. Do not label browser-submitted records as reproduced: maintainers must reproduce and inspect provenance/rights before adding public fixtures or reports. Legacy private records still need separate publication permission; new public candidates carry their own versioned after-review permission.
