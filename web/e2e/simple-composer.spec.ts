...
test("simple entry and examples make no calls or expose configuration", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Compare Jev with another model", exact: true })).toBeVisible();
  await expect(page.getByLabel("What should Jev judge?")).toBeVisible();
  ...
  await expect(page.getByLabel("What should Jev judge?")).not.toHaveValue("");
  await expect(page.getByLabel("What should Jev judge?")).toBeEditable();
  await expect(page.getByLabel("What should Jev judge?")).toBeFocused();
  await expect(page.locator("textarea:visible")).toHaveCount(1);
  await expect(page.getByRole("button", { name: "Start blind comparison" })).not.toBeVisible();
...
test("simple comparison reviews cost before identical deterministic judge inputs", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("What should Jev judge?").fill(content);
  ...
  await expect(page.getByRole("button", { name: "Compare models", exact: true })).toHaveCount(0);
  ...
  await expect(page.getByLabel("Opponent tier")).toBeVisible();
  await expect(page.getByRole("button", { name: "Start blind comparison" })).toBeEnabled();
...
test("simple and advanced drafts survive mode changes", async ({ page }) => {
  ...
  await page.goto("/");
  await page.getByLabel("What should Jev judge?").fill("My simple draft");
  ...
  await page.getByRole("button", { name: "Task options", exact: true }).click();
  await expect(page.getByLabel("What should Jev judge?")).toHaveValue("My simple draft");
  await page.getByRole("button", { name: "Task options", exact: true }).click();
