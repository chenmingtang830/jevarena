...
export const AUTO_REVIEW = process.env.NEXT_PUBLIC_AUTO_REVIEW_ENABLED === "true";
...
export function PublicContributionChoice({ checked, onChange, live = false }: { checked: boolean; onChange: (value: boolean) => void; live?: boolean }) {
  ...
  return <section className="public-contribution-choice" aria-label="Contribution preference">
    <label className="inline-flex min-h-11 cursor-pointer items-center gap-2 font-normal"><Checkbox checked={checked} onChange={event => onChange(event.target.checked)} /><span>Contribute publicly</span></label>
    <p className="hint">{checked
      ? AUTO_REVIEW
        ? `${live ? "Voting" : "Submit & reveal"} sends this contribution to Vercel/Jev for screening and publishes it if it passes. No API key. Uncheck to opt out.`
