...
describe("email authentication boundaries",()=>{
  ...
  it("guest deployment sends no email",async()=>{expect((await POST(request("email=x%40example.test&policy=2026-09-19"))).status).toBe(503);expect(state.send).not.toHaveBeenCalled();});
